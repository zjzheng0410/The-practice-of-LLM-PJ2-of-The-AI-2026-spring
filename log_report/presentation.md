...
我先把“评估可信”这件事修好，再让模型学会写推理，最后用投票把不同模型会做的题拼起来。
...
这次实验最重要的经验不是某个参数，而是先把实验闭环做出来。baseline 让我发现旧验证集不可信，valid_v2 让后面的分数有了参照，CoT 让模型从猜答案变成写过程，投票把不同模型的少数优势拼起来。最终线上分数从 0.140125 到 0.252375，主要提升来自 CoT 和投票。后续如果继续做，重点不应该是盲目再训，而是把错题分析、题型加权投票和更细的 DPO 数据接起来。
